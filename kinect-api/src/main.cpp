...
kinect_websocket *kinectWebsocketPointer;
        ...
        for (int z = 0; z < NUI_SKELETON_COUNT; ++z) {
            ...
            if (skeleton.eTrackingState == NUI_SKELETON_TRACKED) {
                ...
                for (int i = 0; i < NUI_SKELETON_POSITION_COUNT; ++i) {
                    ...
                    std::cout << "[" << i << "](" << skeletonPosition[i].x << "," << skeletonPosition[i].y << ","
                              << skeletonPosition[i].z << ")" << std::endl;
...
void drawKinectData() {
    ...
    kinectWebsocketPointer->setJson(&jsonData);
...
void runWebserver(){
    ...
    kinectWebsocketPointer = &kinectWebsocket;
    kinectWebsocket.run();
}
