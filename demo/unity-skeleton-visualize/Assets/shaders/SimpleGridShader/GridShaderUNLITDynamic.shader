Shader "PDT Shaders/TestGridUNLITDynamic"
{
	Properties
	{
			...
			Pass
			{
				...
				fixed4 frag(v2f i) : SV_Target
				{
					...
					if (round(_SelectCell) == 1.0 && id.x == _SelectedCellX && id.y == _SelectedCellY)
					{
						...
						color = float4(_LineColor.x, _LineColor.y * sin(_Time.y * _Speed) + _LineColor.y * 0.5f, _LineColor.z, _LineColor.w);
					}

					if (frac(uv.x*gsize) <= _LineSize * sin(_Time.w * _Speed) || frac(uv.y*gsize) <= _LineSize * sin(_Time.w * _Speed))
					{
						brightness = _LineColor.w;
						color = float4(_LineColor.x * sin(_Time.y * _Speed) + _LineColor.x * 0.5f, _LineColor.y * sin(_Time.y * _Speed) + _LineColor.y * 0.5f , _LineColor.z * sin(_Time.y * _Speed) + _LineColor.z * 0.5f, _LineColor.w );
					}
