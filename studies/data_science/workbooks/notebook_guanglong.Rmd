...
params: 
  ...
  data_import_path:
    value: "res/imu_and_kinect_with_euler_and_head_2_raw.csv"
    input: file
  output_path: 
    value: "D:/Projekte/Digital-Identity/studies/data_science/workbooks/plots/real_data/"
  T_s:
    value: 0.014
    input: numeric
  x_initial_state_q0:
...
plot(imkDf$imu_pitch_velocity[1:oMTMax] ~ imkDf$t[1:oMTMax], pch  = 4, main = expression("Orientation Kalman for" ~ omega[x]), ylab = expression(omega[x] ~ " in rad/s"), xlab="t in s", type='l', ylim=c(
...
plot(imkDf$imu_roll_velocity[1:oMTMax] ~ imkDf$t[1:oMTMax], pch  = 4, main = expression("Orientation Kalman for" ~ omega[y]), ylab = expression(omega[y] ~ " in rad/s"), xlab="t in s", type='l', ylim=c(
...
plot(imkDf$imu_yaw_velocity[1:oMTMax] ~ imkDf$t[1:oMTMax], pch  = 4, main = expression("Orientation Kalman for" ~ omega[z]), ylab = expression(omega[z] ~ " in rad/s"), xlab="t in s", type='l', ylim=c(
...
orientation_measurement_statistics = data.frame(
  ...
  mean_res_v_y = mean(abs(imkDf$imu_roll_velocity[1:oMTMax] - fkfGuanglongOrientationModel$att[6,1:oMTMax])),
  ...
  mean_res_v_z = mean(abs(imkDf$imu_yaw_velocity[1:oMTMax] - fkfGuanglongOrientationModel$att[7,1:oMTMax]))
)
...
acceleration_measurement_statistics = data.frame(
  ...
  mean_res_a_x = mean(abs(imkDf$imu_acceleration.x[1:aMTMax] - fkfGuanglongAccelerationModel$att[3,1:aMTMax])),
  ...
  mean_res_a_y = mean(abs(imkDf$imu_acceleration.y[1:aMTMax] - fkfGuanglongAccelerationModel$att[6,1:aMTMax])),
  ...
  mean_res_a_z = mean(abs(imkDf$imu_acceleration.z[1:aMTMax] - fkfGuanglongAccelerationModel$att[9,1:aMTMax]))
)
