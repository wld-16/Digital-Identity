...
public class FetchOrientation : MonoBehaviour
{
    ...
    [SerializeField]
    private AxisMappingEntry[] mapping = new AxisMappingEntry[3]
    {
        ...
        if (applyToTransform)
        {
            ...
            switch (_orientationSource)
            {
                case OrientationSource.ACCELEROMETER:
                    ...
                    output = Quaternion.Euler(mapping.Select(entry => entry.applyMapping(toAxis)).Aggregate((leftVector3, rightVector3) => leftVector3 + rightVector3));
                    break;
                case OrientationSource.GYROSCOPE:
                    output = fetchedQuaternion;
                    break;
                case OrientationSource.IMUAXIS:
                    Vector3 axis = mapping.Select(entry => entry.applyMapping(fetchedOrienatationAxis)).Aggregate((leftVector3, rightVector3) => leftVector3 + rightVector3);
                    output = Quaternion.Euler(axis);
                    break;
            ...
            transform.rotation = output * orientationOffset;    
        }
    }
}
...
{
    X,Y,Z,X_NEGATIVE,Y_NEGATIVE,Z_NEGATIVE,ZERO
}
