...
params: 
  ...
  output_path: 
    value: "D:/Projekte/Digital-Identity/data_science/workbooks/plots/simulation_rotate_vz_and_accelerate_forward/"
  T_s:
    value: 0.014
    input: numeric
  sample_size: 
...
kalmanOrientation = data.frame(
  w = simulatedfkfGuanglongOrientationModel$att[1,1:simulatedMax], 
  x = simulatedfkfGuanglongOrientationModel$att[2,1:simulatedMax],
  y = simulatedfkfGuanglongOrientationModel$att[3,1:simulatedMax],
  z = simulatedfkfGuanglongOrientationModel$att[4,1:simulatedMax])

base_gravity = data.frame(x = rep(0,times=simulatedMax), y = rep(0,times=simulatedMax), z = rep(9.81,times=simulatedMax))

normalizedQuaternionOrientation = normalizeQuaternion(kalmanOrientation$w, kalmanOrientation$x, kalmanOrientation$y, kalmanOrientation$z)

gravity = rotate_vector_by(base_gravity, normalizedQuaternionOrientation)
gravity[,1] = gravity[,1]
gravity[,2] = gravity[,2]
gravity[,3] = gravity[,3]
#gravityVector = rotateVectorByQuaternion(data.frame(x = 0, y = 0, z = 9.81), simulation_rotation_quaternion)
...
simulatedfkfGuanglongAccelerationModel = fkf(
  ...
  #ct = matrix(data = c(0,0,0, -9.81 * Ts**2/2, -9.81), nrow = 6, ncol = 1),
  ct = matrix(data = c(
    -gravity$x * Ts**2/2,
    rep(0,times=simulatedMax),
    -gravity$y * Ts**2/2,
